...
#Organizing dolphin data from Local

```{r setup, include=FALSE}
library(readr)
library(dplyr)

df_dol <- read_csv("C:/Workspace_2MA1/Thesis/Dolphins/csv/df_dol_filtered.csv")

df_dol$location_col[df_dol$location_col=="bpns-birkenfels"] <- "bpns-Birkenfels"
df_dol$location_col[df_dol$location_col=="bpns-Reefballs-Cpower"] <- "bpns-Cpowerreefballs-CPOD"
df_dol$location_col[df_dol$location_col=="bpns-Reefballs-cpower"] <- "bpns-Cpowerreefballs-CPOD"

df_dol <- df_dol %>%
  mutate(time = paste0(df_dol$Date," ",df_dol$Time,":00:00"))

df_dol <- as.data.frame(df_dol)
df_dol[,10] <- as.POSIXct(df_dol[,10], format = "%Y-%m-%d %H:%M:%S", tz="UTC")
df_dol <- subset(df_dol, select = -c(2,3,4,7,8,9))

df_dol <- df_dol %>%
  group_by(location_col,time) %>% 
  summarise(dol_clicks_filtered = sum(dol_clicks_filtered), dol_DPH = sum(dol_DPH)) %>%
  mutate(dol_DPH = if_else(dol_DPH>0,1,0)) %>% 
  as.data.frame() %>%
  filter(time > as.POSIXct("2018-10-9 00:00:00", tz="UTC")) 
colnames(df_dol)[2] <- "day_time"

#check if there are duplicates
x <- df_dol[duplicated(df_dol[,1:4]),]

write_csv(df_dol, "csv/DPH_dol.csv")
```

#Organizing dolphins data from ETN
```{r setup, include=FALSE}
library(readr)
library(dplyr)
load("C:/Workspace_2MA1/Thesis/Dolphins/cpod_df_20180701_20220801_week_hour.Rdata")
df_dol <- cpod1_df[cpod1_df$species=="Dolphins",]
write_csv(df_dol,"C:/Workspace_2MA1/Thesis/Dolphins/csv/DPH_dolphins.csv")
...
df_porpoise00 <- write_csv(df_porpoise00,"C:/Workspace_2MA1/Thesis/Porpoise/csv/DPH_porpoise.csv")
df_porpoise00 <- read_csv("C:/Workspace_2MA1/Thesis/Porpoise/csv/DPH_porpoise.csv")
...
detections <- read_csv("detections.csv")
...
deploy <- as.data.frame(read_csv("csv/reclocations.csv")) #all dates when deployments were active by hour
